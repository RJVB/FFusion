 ...
 *  Backported from lavc 54.86.100 by RJVB
 */
...
#define RGB_PIXEL_FORMATS   PIX_FMT_RGB24,PIX_FMT_RGBA,PIX_FMT_RGB48
#define GRAY_PIXEL_FORMATS  PIX_FMT_GRAY8,PIX_FMT_GRAY16
...
#define av_pix_fmt_desc_get(pix_fmt)	(&av_pix_fmt_descriptors[pix_fmt])
...
static inline void libopenjpeg_copy_to_packed8(AVCodecContext *avctx, AVFrame *picture, opj_image_t *image) {
    uint8_t *img_ptr;
    int index, x, y, c;
    for (y = 0; y < avctx->coded_height; y++) {
        index = y*avctx->coded_width;
        img_ptr = picture->data[0] + y*picture->linesize[0];
        for (x = 0; x < avctx->coded_width; x++, index++) {
...
static inline void libopenjpeg_copy_to_packed16(AVCodecContext *avctx, AVFrame *picture, opj_image_t *image) {
    uint16_t *img_ptr;
    int index, x, y, c;
    int adjust[4];
                                    ...
                                    AVPacket *avpkt)
{
    ...
    *data_size = 0;
    ...
    case 1:
        if (ispacked) {
            libopenjpeg_copy_to_packed8(avctx, picture, image);
        } else {
    ...
    case 8:
        if (ispacked) {
            libopenjpeg_copy_to_packed16(avctx, picture, image);
        }
        break;
    default:
...
static const AVOption options[] = {
    { "lowqual", "Limit the number of layers used for decoding",    OFFSET(lowqual), FF_OPT_TYPE_INT, 0, 0, INT_MAX, VD },
    { NULL },
};
